...
class Fetcher(object):
    ...
    def working(self, priority: int, url: str, keys: object, deep: int, repeat: int, proxies=None) -> (int, bool, object):
        """
    ...
    def url_fetch(self, priority: int, url: str, keys: object, deep: int, repeat: int, proxies=None) -> (int, bool, object):
        """
        fetch the content of a url, you can rewrite this function, parameters and return refer to self.working()
        """
        response = requests.get(url, headers={}, proxies=proxies, timeout=(3.05, 10))
        if response.history:
